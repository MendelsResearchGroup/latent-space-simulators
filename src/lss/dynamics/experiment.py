...
from .autoencoder import (
    ...
    NodeDeltaReference8AutoEncoder,
    NodeDeltaReference16AutoEncoder,
    NodeDeltaCorrectedReference16AutoEncoder,
    NodeDeltaMessagePassingReference16AutoEncoder,
    NodeDeltaSpatialDecoderReference16AutoEncoder,
...
from .propagator import make_latent_propagator
...
DEFAULT_PROPAGATOR_CHECKPOINT_METRIC = "val_loss"
...
def _autoencoder_class(model_type: str):
    ...
    if model_type == "attention_reference8":
        return NodeDeltaReference8AutoEncoder
    if model_type == "attention_reference16":
        return NodeDeltaReference16AutoEncoder
    if model_type == "attention_reference16_corrected":
        return NodeDeltaCorrectedReference16AutoEncoder
    if model_type == "message_passing_reference16":
        return NodeDeltaMessagePassingReference16AutoEncoder
    if model_type == "attention_reference16_spatial_decoder":
        return NodeDeltaSpatialDecoderReference16AutoEncoder
...
def latent_experiment_cache_key(source_spec: dict, cfg: dict) -> str:
    ...
    payload = {
        ...
        "cache_version": 4,
        "default_propagator_checkpoint_metric": DEFAULT_PROPAGATOR_CHECKPOINT_METRIC,
    }
...
def train_latent_experiment(source_spec: dict, cfg: dict, *, device) -> dict:
        ...
        context_dim=(
            (
                (
                    ...
                    else int(getattr(ae_model, "reference_dim", params["hidden_size"]))
                    * (
                        4
                        if str(params.get("propagator_context_pool", "mean")).lower()
    ...
    rollout_checkpoint_mode = str(
        params.get(
            ...
            "min" if rollout_checkpoint_metric in {None, "val_loss"} else "max",
        )
    ).lower()
    params["propagator_checkpoint_metric"] = rollout_checkpoint_metric
    params["propagator_checkpoint_mode"] = rollout_checkpoint_mode
    if bool(params.get("propagator_rollout_eval_every_epoch", False)) or rollout_checkpoint_metric not in {None, "val_loss"}:

...
def test_load_dataset_keeps_raw_stiffness_under_coordinate_normalization(tmp_path) -> None:
...
def test_reference_box_normalization_uses_one_fixed_trajectory_map() -> None:
    ...
    torch.testing.assert_close(
        reference.x,
        torch.tensor([[-0.5, -0.4], [0.5, -0.4], [-0.5, 0.4]]),
    )
    torch.testing.assert_close(
        current.x,
        torch.tensor([[-0.4, -0.48], [0.4, -0.48], [-0.4, 0.48]]),
    )
    torch.testing.assert_close(
        reference.box_tensor, torch.tensor([2.0, 2.0])
    )
    torch.testing.assert_close(
        current.box_tensor, torch.tensor([1.6, 2.4])
    )
    torch.testing.assert_close(
        current.edge_attr[:, 2], torch.tensor([0.8, 0.96])
    )
    torch.testing.assert_close(
        current.edge_attr[:, 3], torch.tensor([0.1, 0.25])
    )
    torch.testing.assert_close(reference.edge_attr[:, 3], torch.tensor([0.1, 0.25]))
...
def test_reference_box_normalization_keeps_inverse_square_stiffness_raw() -> None:
    edge_index = torch.tensor([[0, 0], [1, 2]])
    ...
    torch.testing.assert_close(graph.edge_attr[:, 3], stiffness)

#!/usr/bin/env python3
"""Post-fit train-split endpoint p-ratio evaluation of a frozen AE.

This is deliberately a diagnostic only: it reloads an immutable checkpoint,
uses only its declared training LJ trajectories, and never trains or selects.
"""
...
def evaluate(args):
    ...
    groups = []
    train_data, _, _, _ = resolve_train_val_test(recipe['source'], params, split_seed=params['split_seed'])
    for spec in specs:
        if spec['name'] != 'lj_noisy':
            continue
        sims = [sim for sim in train_data if str(sim[0].source_name) == spec['name']]
        ids = list(spec['split_indices']['train'])
        assert len(sims) == len(ids) == 60 and len(set(ids)) == 60
        groups.append(('train', spec, sims, ids))
    rows, schema = [], []
    with torch.no_grad():
    ...
    # The training rows have no prior coordinate table.  The evaluator's
    # physical inversion, preserved context positions, and exact side-membership
    # checks above are retained from the independently audited validation code.
    max_delta = None
    ...
    (out/'recipe.json').write_text(json.dumps({'checkpoint': str(checkpoint), 'training_recipe': str(recipe_file),
        ...
        'split': 'train', 'final_test_used': False, 'training_performed': False, 'selection_performed': False,

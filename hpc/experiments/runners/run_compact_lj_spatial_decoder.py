...
VARIANTS = {
    "spatial_r16_d2": ("attention_reference16_spatial_decoder", 2, True),
    "spatial_r16_d4": ("attention_reference16_spatial_decoder", 4, True),
    "spatial_r16_d8": ("attention_reference16_spatial_decoder", 8, True),
    "ljonly_spatial_r16_d4": ("attention_reference16_spatial_decoder", 4, False),
}
...
def build_recipe(args: argparse.Namespace, manifest: dict, output: Path) -> tuple[dict, dict, dict]:
    ...
    source = {
        "dataset_name": "compact_lj_spatial_decoder_shared" if shared else "compact_lj_spatial_decoder_lj_only",
        "source_name": "reid_lowT_lj" if shared else "lj_only",
        "label": "spatial-decoder shared Reid + low-T + noisy-LJ" if shared else "spatial-decoder noisy-LJ only",
        "path": specs[0]["path"],
...
def main() -> None:
    ...
    output = RESULTS_ROOT / "compact_lj_spatial_decoder" / name

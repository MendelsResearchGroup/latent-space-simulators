...
def optimize(out,source):
    ...
    p=out/'prepared'/'depablo_mixed_temp'/'prepare'/'prepared.pt';data=torch.load(p,weights_only=False,map_location='cpu');data['ae'].eval()
    for z in data['ae'].parameters():z.requires_grad_(False)
    prep=json.loads((p.parent/'preparation.json').read_text()); ids=prep['cohort_ids_in_order']
    if len(ids)!=30 or set(data['temperatures'])!=set(ids): raise ValueError('expected exactly the frozen 30-ID mixed-T cohort')
    recipes=[('A',.02,.25,50),('B',.005,.25,150),('C',.02,1.,150)]
    recipe=dict(stage='optimize',status='running',source=source,source_ids=ids,prepared_sha256=sha(p),script_sha256=sha(__file__),code_manifest_sha256=sha(CODE/'hashes.json'),cohort_sha256=prep['cohort_sha256'],cohort_path=prep['cohort_path'],model_seed=786,policy='z1',arms=[dict(name=n,lr=lr,log_stiffness_bound=b,patience=pat) for n,lr,b,pat in recipes],max_steps=1200,optimizer='torch.optim.Adam default betas=(.9,.999), eps=1e-8',finite_difference_epsilon=.01,selection='strict lowest frozen z1 score among initial and all feasible post-update iterates separately for each predeclared recipe',early_stopping='recipe-specific consecutive updates without score decrease >1e-5 below patience_reset_best; strict global best still updates for smaller decreases',counts=dict(ml=90,random=90,original=30,total=210),job_id=os.environ.get("PBS_JOBID"),split=prep['split'],geometry_constraints="fixed box/topology/boundary nodes; radius4 equals <=1.0 shortest original incident bond; lengths 0.25..1.75 original; pair separation >=0.1 original minimum",gradient_scaling="raw gradients; inactive/fixed entries masked and active stiffness gradient centered; no block-max normalization",preparation=prep,no_response_targets=True,no_external_processes=True,feasible_search='after each Adam proposal, project blocks then deterministically halve the whole proposal interpolation up to16 times until feasible; rejected proposals retain Adam momentum and current feasible parameters',control='per recipe, signed centered stiffness permutation and movable q permutation, seed123+ID; geometry-only q halving, no norm matching or score selection',reuse='none; all 210 graphs newly frozen from pristine direct mixed-T frame0 references')
    ...
    for index in ids:
        ...
        def save(name,u,q,kind,mode,bound,pred,extra={}):
            ...
            gp,ep=folder/f'{name}.pt',folder/f'{name}_edit.pt';torch.save(physical,gp);torch.save(dict(log_stiffness=u.detach(),position_q=q.detach()),ep);records.append(dict(source=source,index=index,temperature=float(data['temperatures'][index]),variant=name,kind=kind,policy='z1',mode=mode,bound=bound,model_seed=786,predicted_p=float(pred),scale_x=float(data['scales'][index]),physicalpath=str(gp.resolve()),sha256=sha(gp),edit_file=str(ep.resolve()),edit_sha256=sha(ep),**extra))
        ...
        for arm,lr,bound,stop_patience in recipes:
            mode,radius='joint',4.
            RADIUS=radius
            u=torch.zeros_like(k0,requires_grad=True);q=torch.zeros_like(x0,requires_grad=True); optimizer=torch.optim.Adam([u,q],lr=lr)
            ...
            for step in range(1,1201):
                ...
                if patience>=stop_patience: reason=f'patience_{stop_patience}';break
            actual_steps=len(hist)
            name=f'joint_{arm}_r{radius:g}_b{bound:g}';pd.DataFrame(hist).to_csv(folder/f'{name}_history.csv',index=False);dump(folder/f'{name}_completion.json',dict(initial_score=initial,best_score=best,actual_steps=actual_steps,stop_reason=reason,patience=patience,arm=arm,learning_rate=lr));save(name,bu,bq,'ml',mode,bound,best,dict(position_radius=radius,actual_steps=actual_steps,stop_reason=reason,arm=arm,learning_rate=lr,patience_limit=stop_patience))
            if mode=='joint':
                ...
                save(name+'_permuted',ru,cq,'random','joint',bound,score(data,ref,raw,ru,cq),dict(control_seed=123+index,position_radius=radius,control_shrink_factor=factor,control_q_norm_ratio=float(cq.norm()/bq.norm().clamp_min(EPS)),arm=arm,learning_rate=lr,patience_limit=stop_patience))
    dump(out/'gradient_checks.json',checks)
    ...
    if len(records)!=210:raise RuntimeError('unexpected frozen-design count')
...
def optimize_all(out):
    ...
    optimize(out,'depablo_mixed_temp')
    sub=json.loads((out/'frozen_designs.json').read_text())
    assert sub['recipe_sha256']==sha(out/'recipe.json') and len(sub['records'])==210
    protocol_path=out/'thermal_protocol.json'; protocol=json.loads(protocol_path.read_text())
    protocol.update(manifest_sha256=sha(out/'frozen_designs.json'),metric_sha256=sha(out/'code/thermal_metric.py'),verifier_sha256=sha(out/'code/verify.py'),expected_designs=210,expected_evaluations=630)
...
if __name__=='__main__':
    a=argparse.ArgumentParser();a.add_argument('--stage',choices=['optimize'],required=True);a.add_argument('--output',type=Path,required=True);x=a.parse_args();x.output.mkdir(parents=True,exist_ok=True)
    if x.stage=='optimize':

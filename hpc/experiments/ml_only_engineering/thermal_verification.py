...
'''
def verify(out,temperature=None):
 torch.set_num_threads(1);out=Path(out);m=json.loads((out/'frozen_designs.json').read_text());assert m['recipe_sha256']==sha(out/'recipe.json');assert len(m['records'])==24
 ...
 selected=[r for r in m['records'] if temperature is None or float(r['temperature'])==float(temperature)]
 assert len(selected)==(24 if temperature is None else 4)
 tag='all' if temperature is None else f'T{temperature:g}';folder=out/'thermal_physics'/tag;folder.mkdir(parents=True,exist_ok=True)
 if (folder/'verification_recipe.json').exists():raise RuntimeError('Refuse overwrite of thermal verification attempt')
 dump(folder/'verification_recipe.json',dict(job_id=os.environ.get('PBS_JOBID'),manifest_sha256=sha(out/'frozen_designs.json'),protocol_sha256=sha(protocol_path),started_at_unix=time.time(),temperature=temperature,expected=len(selected)*len(p['thermal_seeds']),feedback_to_optimizer=False))
 rows=[]
 for r in selected:
...
if __name__=='__main__':
 a=argparse.ArgumentParser();a.add_argument('--output',type=Path,required=True);a.add_argument('--temperature',type=float);args=a.parse_args();verify(args.output,args.temperature)

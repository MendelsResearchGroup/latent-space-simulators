...
def optimize(out,source):
    ...
    for index in ids:
        ...
        for mode,bound,radius in [('positions',0.,2.),('positions',0.,4.),('joint',.25,4.)]:
            RADIUS=radius
    ...
    if len(records)!=10:raise RuntimeError('unexpected per-source frozen-design count')
...
def optimize_all(out):
    ...
    dump(out/'recipe.json',dict(status='completed',job_id=os.environ.get('PBS_JOBID'),source_recipes=source_recipes,script_sha256=sha(__file__),counts=dict(originals=4,ml=12,random=4,total=20),screening_recipe_sha256=sha(out.parent/'screening/recipe.json')))

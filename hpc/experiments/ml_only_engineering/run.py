...
NETWORKS = {'reid': [2, 53, 59], 'depablo_low_temp': [70, 78, 96]}
...
def optimize(out):
    ...
    recipe = dict(stage='optimize', status='running', job_id=os.environ.get('PBS_JOBID'),
        ...
        networks=NETWORKS, model_seed=786, policies=['z1', 'ridge2d'], bounds=[0.7, 2.0],
        adam_steps=250, learning_rate=0.02, target=-0.15,
        objective='relu(predicted_p + 0.15)^2 + 0.001*mean(active_log_edit^2) + 0.01*relu(standardized_latent_distance-3)^2',
        selection='minimum ML objective over initial and 250 iterates; no physics or target-network response consulted',
        ...
        split='six previously studied validation-development networks; not untouched test',
        counts=dict(ml_designs=24, random_controls=12, originals=6),
        ...
        hypothesis='remove the near-uniform stiffness scaling found in v1 edits; preserve geometric mean of active spring stiffness')
        ...
        for index in indices:
            ...
            for bound in (0.7,2.0):
                for policy in ('z1','ridge2d'):
                    ...
                    u=torch.nn.Parameter(torch.zeros_like(k0)); opt=torch.optim.Adam([u],lr=.02)
                    ...
                    for step in range(251):
                        ...
                        loss=torch.relu(pred+.15).square()+.001*u[active].square().mean()+.01*torch.relu(distance-3).square()
                        ...
                        # Adam's update onto sum(u_active)=0 intersect the box.
    ...
    assert len(records)==42 and sum(r['kind']=='ml' for r in records)==24

...
def optimize(out,source):
    ...
    recipe=dict(stage='optimize',status='running',source=source,source_ids=ids,prepared_sha256=sha(p),script_sha256=sha(__file__),code_manifest_sha256=sha(CODE/'hashes.json'),model_seed=786,policy='z1',arms=['positions_r2_b0','positions_r4_b0'],steps=1200,step_sizes=[.05,.025,.0125,.00625],selection='lowest frozen z1 readout among valid ML-only iterates',counts=dict(ml=4,random=2,original=2,total=8),job_id=os.environ.get("PBS_JOBID"),split="highest original-response train/calibration/validation development cases; no final test",geometry_constraints="fixed box/topology/boundary nodes; movement <=0.5 or1.0 shortest original incident bond; lengths 0.25..1.75 original; pair separation >=0.1 original minimum",gradient_scaling="center stiffness; normalize spring and geometry blocks separately",preparation=json.loads((p.parent/"preparation.json").read_text()),no_response_targets=True,no_external_processes=True,feasible_search='up to16 node-mask retries per iteration; mask endpoints of invalid bonds/close pairs from smallest invalid backtrack; reset mask next iteration',control='permuted movable-node q field from positions_r4, geometry-only shrinking; not necessarily norm matched',prior_study='study_v2 ML histories still improve at400; study_v2 physical verification partially observed before declaration; separate reused-development budget test, restart from pristine originals')
    ...
    for index in ids:
        ...
        for mode,bound,radius in [('positions',0.,2.),('positions',0.,4.)]:
            RADIUS=radius
            ...
            if mode=='positions' and radius==4:
                ...
                for attempt in range(25):
                    ...
                    if valid(ref,raw,u0,cq,sides): break
                    factor*=.5
                else: raise RuntimeError('No feasible geometry control')
                save(name+'_permuted',u0,cq,'random','positions',0,score(data,ref,raw,u0,cq),dict(control_seed=123+index,position_radius=radius,control_shrink_factor=factor,control_q_norm_ratio=float(cq.norm()/bq.norm().clamp_min(EPS))))
    dump(out/'gradient_checks.json',checks)
    ...
    if len(records)!=8:raise RuntimeError('unexpected per-source frozen-design count')
...
def optimize_all(out):
    ...
    dump(out/'recipe.json',dict(status='completed',job_id=os.environ.get('PBS_JOBID'),source_recipes=source_recipes,script_sha256=sha(__file__),counts=dict(originals=4,ml=8,random=4,total=16),screening_recipe_sha256=sha(out.parent/'screening/recipe.json')))

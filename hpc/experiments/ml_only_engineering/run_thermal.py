...
def check(data,ref,raw,u,q,active,movable,sides,index):
    ...
    for block in ('spring','position'):
        ...
        rel=abs(fd-exact)/max(abs(fd),abs(exact),EPS); checks.append(dict(index=index,block=block,autograd=exact,finite_difference=fd,finite_difference_epsilon=.01,relative_error=rel))
    return checks
def optimize(out,source):
    ...
    p=out.parent.parent/'study_v1'/'prepared'/source/'prepare'/'prepared.pt';data=torch.load(p,weights_only=False,map_location='cpu');data['ae'].eval()
    for z in data['ae'].parameters():z.requires_grad_(False)
    ids=sorted(data['references']);
    if len(ids)!=6: raise ValueError('Expected six temperature-stratified validation references.')
    recipe=dict(stage='optimize',status='running',source=source,source_ids=ids,prepared_sha256=sha(p),script_sha256=sha(__file__),code_manifest_sha256=sha(CODE/'hashes.json'),model_seed=786,policy='z1',arms=['positions_r4_b0','joint_r4_b0.25'],steps=1200,step_sizes=[.05,.025,.0125,.00625],selection='lowest frozen z1 readout among valid ML-only iterates',counts=dict(ml=12,random=6,original=6,total=24),job_id=os.environ.get("PBS_JOBID"),split="mixed-T validation, response-blind six-temperature selection, excluded from frozen AE/readout training; no final test",geometry_constraints="fixed box/topology/boundary nodes; movement <=0.5 or1.0 shortest original incident bond; lengths 0.25..1.75 original; pair separation >=0.1 original minimum",gradient_scaling="center stiffness; normalize spring and geometry blocks separately",preparation=json.loads((p.parent/"preparation.json").read_text()),no_response_targets=True,no_external_processes=True,feasible_search='up to16 node-mask retries per iteration; mask endpoints of invalid bonds/close pairs from smallest invalid backtrack; reset mask next iteration',control='permuted q and centered log-stiffness fields from joint bound0.25; geometry-only q shrinking, stiffness norm preserved',prior_study='first mixed-T engineering validation; low-T historical z1 readout zero-shot transfer without thermal-response fitting, not autonomous position prediction')
    ...
    for index in ids:
        ...
        def save(name,u,q,kind,mode,bound,pred,extra={}):
            ...
            gp,ep=folder/f'{name}.pt',folder/f'{name}_edit.pt';torch.save(physical,gp);torch.save(dict(log_stiffness=u.detach(),position_q=q.detach()),ep);records.append(dict(source=source,index=index,variant=name,kind=kind,policy='z1',mode=mode,bound=bound,model_seed=786,temperature=float(data['temperatures'][index]),predicted_p=float(pred),scale_x=float(data['scales'][index]),physicalpath=str(gp.resolve()),sha256=sha(gp),edit_file=str(ep.resolve()),edit_sha256=sha(ep),**extra))
    ...
    if len(records)!=24:raise RuntimeError('unexpected per-source frozen-design count')
...
def optimize_all(out):
    ...
    for source in ['depablo_mixed_temp']:
        sub=out/source
    ...
    dump(out/'recipe.json',dict(status='completed',job_id=os.environ.get('PBS_JOBID'),source_recipes=source_recipes,script_sha256=sha(__file__),counts=dict(originals=6,ml=12,random=6,total=24),cohort='fixed first validation ID at each of six temperatures, no response screening'))
    dump(out/'frozen_designs.json',dict(recipe_sha256=sha(out/'recipe.json'),frozen_at_unix=time.time(),records=records))
def verify(out):
    helper=out/'code/thermal_verification.py'
    if not helper.exists(): raise RuntimeError('Thermal protocol not yet resolved; do not use quasistatic endpoint verification')
    spec=importlib.util.spec_from_file_location('thermal_verification',helper);mod=importlib.util.module_from_spec(spec);spec.loader.exec_module(mod);mod.verify(out)

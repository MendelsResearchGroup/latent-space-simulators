...
def optimize(out):
    ...
    recipe = dict(stage='optimize', status='running', job_id=os.environ.get('PBS_JOBID'),
        ...
        counts=dict(ml_designs=12, random_controls=6, originals=6),
        ...
        hypothesis='v2 often stops at predicted target before using the edit budget; use signed latent direction only and full fixed budget, preserving geometric mean stiffness')
    ...
    assert len(records)==24 and sum(r['kind']=='ml' for r in records)==12

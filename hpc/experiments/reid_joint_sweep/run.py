...
def optimize(out,source):
    ...
    config=json.loads((out/'config.json').read_text());p=Path(config['prepared_path']);data=torch.load(p,weights_only=False,map_location='cpu');data['ae'].eval()
    for z in data['ae'].parameters():z.requires_grad_(False)
    prep=json.loads((p.parent/'preparation.json').read_text()); ids=config['ids']
    if ids!=prep['cohort_ids_in_validation_order']: raise ValueError('cohort order mismatch')
    recipe=dict(stage='optimize',status='running',source=source,source_ids=ids,prepared_sha256=sha(p),script_sha256=sha(__file__),code_manifest_sha256=sha(CODE/'hashes.json'),config=config,model_seed=786,policy='z1',arms=['joint_r4_b'+str(config['log_stiffness_bound'])],max_steps=1200,optimizer=f"torch.optim.Adam(lr={config['learning_rate']}, betas=(.9,.999), eps=1e-8)",finite_difference_epsilon=.01,selection='strict lowest frozen z1 score among initial and all feasible post-update iterates',early_stopping=f"{config['patience']} consecutive updates without a score decrease >1e-5 below patience_reset_best; strict global best still updates for smaller decreases",counts=dict(ml=10,random=10,original=0,total=20),job_id=os.environ.get("PBS_JOBID"),split=prep['split'],geometry_constraints="fixed box/topology/boundary nodes; radius4 equals <=1.0 shortest original incident bond; lengths 0.25..1.75 original; pair separation >=0.1 original minimum",gradient_scaling="raw gradients; inactive/fixed entries masked and active stiffness gradient centered; no block-max normalization",preparation=prep,no_response_targets=True,no_external_processes=True,feasible_search=f"{config['feasibility_mode']}: whole-proposal deterministic16 halvings; mask_blocking_nodes additionally retries up to16 node masks after all whole-proposal scales fail; no score acceptance",control='joint signed centered stiffness permutation and movable q permutation, seed123+ID; geometry-only q halving, no norm matching or score selection',reuse='none; every setting begins from pristine references')
    ...
    for index in ids:
        ...
        for mode,bound,radius in [('joint',float(config['log_stiffness_bound']),4.)]:
            RADIUS=radius
            u=torch.zeros_like(k0,requires_grad=True);q=torch.zeros_like(x0,requires_grad=True); optimizer=torch.optim.Adam([u,q],lr=float(config['learning_rate']))
            ...
            for step in range(1,1201):
                ...
                with torch.no_grad():
                    ...
                    accepted=False;accepted_scale=0.;candidate=float('nan');halvings=16;masked=torch.zeros(len(q),dtype=torch.bool);mask_passes=0
                    ...
                    if not accepted and config['feasibility_mode']=='mask_blocking_nodes':
                        for mask_passes in range(1,17):
                            offenders=blocking_nodes(ref,raw,nu,nq)
                            new=offenders & ~masked & ~fixed
                            if not new.any(): break
                            masked |= new; proposal_q[masked]=last_q[masked];project_q_(proposal_q,x0,scale,fixed,lo,hi)
                            for halve in range(17):
                                factor=.5**halve;nu=last_u+(proposal_u-last_u)*factor;nq=last_q+(proposal_q-last_q)*factor;project_u_(nu,active,bound);project_q_(nq,x0,scale,fixed,lo,hi)
                                if valid(ref,raw,nu,nq,sides):
                                    u.copy_(nu);q.copy_(nq);candidate=float(score(data,ref,raw,u,q));accepted=True;accepted_scale=factor;halvings=halve;break
                ...
                hist.append(dict(step=step,predicted_p=candidate,best_score=best,patience_reset_best=reset_best,patience=patience,accepted=accepted,accepted_scale=accepted_scale,halvings=halvings,mask_passes=mask_passes,masked_nodes=int(masked.sum()),strict_best=strict_best,material_improvement=material))
                if patience>=int(config['patience']): reason=f"patience_{config['patience']}";break
    ...
    if len(records)!=20:raise RuntimeError('unexpected frozen-design count')
...
def optimize_all(out):
    ...
    assert sub['recipe_sha256']==sha(out/'recipe.json') and len(sub['records'])==20
...
def verify(out):
    ...
    global_done=Path(config['global_freeze_completed_path'])
    if not global_done.is_file(): raise RuntimeError('global freeze gate missing')
    completion=json.loads(global_done.read_text()); manifest_path=Path(config['global_manifest_path'])
    if completion['total']!=490 or completion['manifest_sha256']!=sha(manifest_path): raise RuntimeError('global freeze gate hash/count mismatch')
    global_rows=json.loads(manifest_path.read_text())['records']
    ...
    frozen={(r['index'],r['variant'],r['sha256'],r['edit_sha256']) for r in global_rows if r['config_id']==config['id']}
    local={(r['index'],r['variant'],r['sha256'],r['edit_sha256']) for r in m['records']}
    if frozen!=local or len(frozen)!=20: raise RuntimeError('setting is not exactly represented in the global frozen manifest')
    ...
    table=pd.DataFrame(rows); baseline=pd.read_csv(config['baseline_results_path']); original=baseline[baseline.kind.eq('original')].set_index(['source','index']).physical_p_ratio
    ...
    if len(rows)!=20: raise RuntimeError(f'unexpected verification count rows={len(rows)}')
    dump(out/'verification_completed.json',dict(total=len(rows),valid=int(table.status.eq('completed').sum()),new_physical_evaluations=20,reused_physical_measurements=0,feedback_to_optimizer=False,manifest_sha256=sha(out/'frozen_designs.json')))
